    ...
    <Card className="w-full max-w-2xl">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-base font-medium">Debt Payment Reminders</Label>
              <p className="text-sm text-muted-foreground">
                Get reminded about upcoming debt payments
              </p>
            </div>
            <Switch
              checked={localPreferences.debtReminders.enabled}
              onCheckedChange={(enabled) => 
                updatePreferenceSection('debtReminders', { enabled })
              }
            />
          </div>
          
          {localPreferences.debtReminders.enabled && (
            <div className="ml-4 space-y-3">
              <div>
                <Label className="text-sm">Days Before Due Date</Label>
                <Input
                  type="number"
                  min="1"
                  max="30"
                  value={localPreferences.debtReminders.daysBefore}
                  onChange={(e) => 
                    updatePreferenceSection('debtReminders', { 
              ...
              <div>
                ...
                <div className="flex gap-4 mt-2">
                  {channelOptions.map((channel) => (
                    <div key={channel.value} className="flex items-center space-x-2">
                      <Checkbox
                        id={`debt-${channel.value}`}
                        checked={localPreferences.debtReminders.channels.includes(channel.value)}
                        onCheckedChange={() => toggleChannel('debtReminders', channel.value)}
                      />
                      <Label htmlFor={`debt-${channel.value}`} className="text-sm">
                        {channel.label}
                      </Label>
                    </div>
                  ))}
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleTestNotification('DEBT_PAYMENT_DUE')}
              >
                <TestTube className="h-4 w-4 mr-2" />
                Test
              </Button>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Get notified about price drops on your wishlist items
              </p>
            </div>
            <Switch
              checked={localPreferences.priceAlerts.enabled}
              onCheckedChange={(enabled) => 
                updatePreferenceSection('priceAlerts', { enabled })
              }
            />
          </div>
          
          {localPreferences.priceAlerts.enabled && (
            <div className="ml-4 space-y-3">
              <div>
                <Label className="text-sm">Price Drop Threshold (%)</Label>
                <Input
                  type="number"
                  min="1"
                  max="50"
                  value={localPreferences.priceAlerts.priceDropPercentage}
                  onChange={(e) => 
                    updatePreferenceSection('priceAlerts', { 
                      priceDropPercentage: parseInt(e.target.value) 
                    })
                  }
                  className="w-20"
                />
              ...
              <div>
                ...
                <div className="flex gap-4 mt-2">
                  {channelOptions.map((channel) => (
                    <div key={channel.value} className="flex items-center space-x-2">
                      <Checkbox
                        id={`price-${channel.value}`}
                        checked={localPreferences.priceAlerts.channels.includes(channel.value)}
                        onCheckedChange={() => toggleChannel('priceAlerts', channel.value)}
                      />
                      <Label htmlFor={`price-${channel.value}`} className="text-sm">
                        {channel.label}
                      </Label>
                    </div>
                  ))}
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleTestNotification('PRICE_ALERT')}
              >
                <TestTube className="h-4 w-4 mr-2" />
                Test
              </Button>
