...
# Development startup script for Permoney
...
echo -e "${BLUE}🚀 Starting Permoney Development Environment...${NC}"
...
# Start backend with transpile-only to ignore type errors temporarily
echo -e "${BLUE}🎯 Starting Backend on port 3001...${NC}"
cd "$PROJECT_ROOT/backend"
npx ts-node --transpile-only src/simple-main.ts > /tmp/permoney-backend.log 2>&1 &
BACKEND_PID=$!
...
echo ""
echo -e "${GREEN}🎉 Permoney Development Environment is running!${NC}"
echo ""
